...
def t_student(request,assign_id):
    ...
    for stud in ass.class_id.student_set.all():
        try:
            ...
            a.save()
        att_list.append(a)
    return render(request,'info/t_student.html',{'att_list':att_list})
@login_required
...
def confirm(request,ass_c_id):
    ...
    for i,s in enumerate(cl.student_set.all()):
        ...
        if assc.status == 1:
            try:
                a = Attendance.objects.get(course=cr,student=s,date=assc.date,attendancelass=assc)
                a.status = status
                a.save()
            except Attendance.DoesNotExist:
                a = Attendance(course=cr,student=s,status=status,date=assc.date,attendancelass=assc)
                a.save()
        else:
